...
def backtest(data, short_window=15, long_window=55, initial_balance=10000, leverage=1):
    ...
    highest_loss_per_trade = 0  # Track highest loss per trade properly
    trade_open_price = 0
    lowest_price_during_trade = float('inf')  # Track lowest price for long trades
    highest_price_during_trade = float('-inf')  # Track highest price for short trades
    ...
    for i in range(long_window, len(data)):
        ...
        # Buy signal (short MA > long MA and RSI < 30)
        if short_ma > long_ma and position <= 0:
            if position < 0:  # Closing short position
                ...
                position = 0
                print(" ")
                print(f"CLOSE SHORT at {exit_price}, Profit: {profit:.2f}, Max Loss During Trade: {highest_loss_per_trade:.2f}")
            ...
            lowest_price_during_trade = entry_price  # Reset for new trade
            highest_loss_per_trade = 0  # Reset highest loss tracker
            print(" ")
            print(f"BUY at {entry_price}")
            print(f"BALANCE is {balance}")
        
        # Sell/Short signal (short MA < long MA and RSI > 70)
            ...
            if position > 0:  # Closing long position
                ...
                position = 0
                print(" ")
                print(f"SELL at {exit_price}, Profit: {profit:.2f}, Max Loss During Trade: {highest_loss_per_trade:.2f}")
                print(f"BALANCE is {balance}")
            ...
            trade_open_price = entry_price
            highest_price_during_trade = entry_price  # Reset for new trade
            highest_loss_per_trade = 0  # Reset highest loss tracker
            print(" ")
            print(f"SHORT at {entry_price}")
            print(f"BALANCE is {balance}")
        
        # Ensure highest_loss_per_trade starts at positive infinity
        if highest_loss_per_trade == float('inf'):
            highest_loss_per_trade = 0  

        # Track the worst loss during the trade
        if position > 0:  # Long position
            lowest_price_during_trade = min(lowest_price_during_trade, data[i][1])
            loss = (lowest_price_during_trade - trade_open_price) * position  # Ensure it's negative for loss
            highest_loss_per_trade = min(highest_loss_per_trade, loss)  # Track most negative value

        elif position < 0:  # Short position
            highest_price_during_trade = max(highest_price_during_trade, data[i][1])
            loss = (trade_open_price - highest_price_during_trade) * position  # Ensure it's negative for loss
            highest_loss_per_trade = min(highest_loss_per_trade, loss)  # Track most negative value
    ...
    if position > 0:
        ...
        print(" ")
        print(f"SELL LONG at {data[-1][1]}, Profit: {profit:.2f}, Max Loss During Trade: {highest_loss_per_trade:.2f}")
    elif position < 0:
        exit_price = data[-1][1]
        profit = (entry_price - exit_price) * abs(position)
